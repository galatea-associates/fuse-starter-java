 ...
 * we've figured out (pending issue #221), we need to manually put a wrapper object around the
 * List in order to get that root element.
 *
 ...
 * a Class as an argument. If we used {@code List<SettlementMission>} in our controller
 * method signature, the argument value would just be List. Inside of those #canRead and #canWrite
 * methods we'd have no way of figuring out what's in the List. As a result, we need this wrapper
 * to know while inside HttpMessageConverter that we have a List of SettlementMissionMessage.
 *
