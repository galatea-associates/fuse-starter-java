...
/**
 * Builds upon spring actuator's web request tracer to capture interesting audit information. We capture some additional
 * timing data as well as the request/response payload (which is missing from spring's default implementation).
 *
 * @author rbasu
 *
 */
...
public class FuseWebRequestTraceFilter extends WebRequestTraceFilter {
	 ...
	 *
	 * @param repository
	 *            the respository where we store our trace
	 * @param properties
	 *            any trace properties
	 * @param pathsToSkip
	 *            a predicate that will return try if we want to a skip a certain url path
	 * @param maxPayloadLength
	 *            the max number of bytes of payload information that we want to capture in the trace
	 */
	public FuseWebRequestTraceFilter(final FuseTraceRepository repository, final TraceProperties properties,
			final Predicate<String> pathsToSkip, final Integer maxPayloadLength) {
	...
	protected void doFilterInternalHelper(final HttpServletRequest request, final HttpServletResponse response,
	...
	protected void enhanceTrace(final Map<String, Object> springTraceInfo, final HttpServletRequest request,
		...
		ContentCachingRequestWrapper requestWapper = WebUtils.getNativeRequest(request,
	...
	protected String getPayload(final ContentCachingRequestWrapper wrapper) {
		return wrapper == null ? "null" : getPayload(wrapper.getContentAsByteArray(), wrapper.getCharacterEncoding());
	}
	...
	/**
	 * Copied this one from the super class. I would have reused the super class's version but alas it was private.
	 *
	 * @author rbasu
	 *
	 */
